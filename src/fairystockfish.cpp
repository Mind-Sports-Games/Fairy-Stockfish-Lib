...
fairystockfish::Piece::Piece(int pt, int color, bool promoted)
    : _pieceInfo{pt}, _color{static_cast<SF::Color>(color)},
      _promoted{promoted} {}
...
void fairystockfish::setUCIOption(std::string name, std::string value) {
  if (SF::Options.count(name))
    SF::Options[name] = value;
  else
...
std::map<std::string, fairystockfish::PieceInfo>
...
bool fairystockfish::validateFEN(std::string variantName, std::string fen,
                                 bool isChess960) {

  return FenValidation::FEN_OK ==
         SF::FEN::validate_fen(fen, SF::variants.find(variantName)->second,
                               isChess960);
}
...
fairystockfish::to960Uci(std::string variantName,
  ...
  for (auto const &move : moves) {
    ...
    std::set_difference(uci960Moves.begin(), uci960Moves.end(),
                        uciMoves.begin(), uciMoves.end(),
                        std::inserter(onlyIn960, onlyIn960.begin()));

    std::vector<std::string> onlyNormalUci;
    std::set_difference(uciMoves.begin(), uciMoves.end(), uci960Moves.begin(),
                        uci960Moves.end(),
                        std::inserter(onlyNormalUci, onlyNormalUci.begin()));
    if (onlyIn960.size() == 1 && onlyNormalUci.size() == 1 &&
    ...
    } else if (onlyIn960.size() == 2 && onlyNormalUci.size() == 2 &&
      ...
      bool isCastle = (startPiece != pieceMap.end() &&
...
void fairystockfish::Position::init(std::string startingFen, bool _isChess960) {
  const Stockfish::Variant *v =
      Stockfish::variants.find(std::string(variant))->second;

  auto newState = std::make_shared<StateNode>();

  std::shared_ptr<Stockfish::Position> p =
      std::make_shared<Stockfish::Position>();
  p->set(v, startingFen, isChess960, &newState->stateInfo,
         Stockfish::Threads.main());
  position = p;
  state = newState;
}

fairystockfish::Position::Position(std::string _variant, bool _isChess960)
    : variant(_variant), isChess960(_isChess960), position{} {
...
fairystockfish::Position::Position(std::string _variant,
                                   std::string startingFen, bool _isChess960)
    : variant(_variant), isChess960(_isChess960), position{} {
  init(startingFen, _isChess960);
}

fairystockfish::Position::SFPositionPtr fairystockfish::Position::copyPosition(
    fairystockfish::Position::SFPositionConstPtr const &ptr) const {
...
Stockfish::Notation
fairystockfish::Position::fromOurNotation(fairystockfish::Notation n) const {
  switch (n) {
  case fairystockfish::NOTATION_DEFAULT:
    return Stockfish::NOTATION_DEFAULT;
  case fairystockfish::NOTATION_SAN:
    return Stockfish::NOTATION_SAN;
  case fairystockfish::NOTATION_LAN:
    return Stockfish::NOTATION_LAN;
  case fairystockfish::NOTATION_SHOGI_HOSKING:
    return Stockfish::NOTATION_SHOGI_HOSKING;
  case fairystockfish::NOTATION_SHOGI_HODGES:
    return Stockfish::NOTATION_SHOGI_HODGES;
  case fairystockfish::NOTATION_SHOGI_HODGES_NUMBER:
    return Stockfish::NOTATION_SHOGI_HODGES_NUMBER;
  case fairystockfish::NOTATION_JANGGI:
    return Stockfish::NOTATION_JANGGI;
  case fairystockfish::NOTATION_XIANGQI_WXF:
    return Stockfish::NOTATION_XIANGQI_WXF;
  }
  return Stockfish::NOTATION_DEFAULT;
}

fairystockfish::Position
fairystockfish::Position::makeMoves(MoveList const &uciMoves) const {
...
std::string fairystockfish::Position::getSAN(std::string uciMove,
                                             Notation notation) const {
  return this->getSANMoves({uciMove}, notation)[0];
}

std::vector<std::string>
fairystockfish::Position::getSANMoves(std::vector<std::string> uciMoves,
                                      Notation ourNotation) const {
  Stockfish::Notation notation = fromOurNotation(ourNotation);
  if (notation == Stockfish::NOTATION_DEFAULT)
    notation =
        Stockfish::default_notation(Stockfish::variants.find(variant)->second);
  ...
  ListOfImmutableStatesPtr tmpStates =
      std::make_shared<std::list<StateInfoPtr>>();
...
std::string fairystockfish::Position::getFEN(bool sFen, bool showPromoted,
                                             int countStarted) const {
  countStarted =
      std::min<unsigned int>(countStarted, INT_MAX); // pseudo-unsigned
...
int fairystockfish::Position::gameResult() const {
  ...
  if (!gameEnd)
    result = position->checkers() ? position->checkmate_value()
...
std::tuple<bool, int>
fairystockfish::Position::isOptionalGameEnd(int countStarted) const {
  countStarted =
      std::min<unsigned int>(countStarted, INT_MAX); // pseudo-unsigned
...
std::tuple<bool, bool>
fairystockfish::Position::hasInsufficientMaterial() const {
  bool wInsufficient =
      Stockfish::has_insufficient_material(Stockfish::WHITE, *position);
  bool bInsufficient =
      Stockfish::has_insufficient_material(Stockfish::BLACK, *position);
...
fairystockfish::Position::piecesOnUciBoard() const {
...
std::map<fairystockfish::Square, fairystockfish::Piece>
fairystockfish::Position::piecesOnBoard() const {
  ...
  for (Stockfish::File f = Stockfish::File::FILE_A; f <= v->maxFile; ++f) {
    for (Stockfish::Rank r = Stockfish::Rank::RANK_1; r <= v->maxRank; ++r) {
      ...
      retVal.insert(
          {static_cast<Square>(s), fairystockfish::Piece(pt, c, promoted)});
    }
  }
  return retVal;
}

std::vector<fairystockfish::Piece>
fairystockfish::Position::piecesInHand() const {
