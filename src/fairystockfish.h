...
enum Square : std::uint8_t {
  ...
  SQUARE_NB = 120,
  SQUARE_BIT_MASK = 127,
  SQ_MAX = 119, // SQUARE_NB - 1 NOTE: this has to be hard coded due to javacpp
                // (or my lack of knowledge of javacpp)
  SQUARE_NB_CHESS = 64,
  SQUARE_NB_SHOGI = 81,
};
...
///------------------------------------------------------------------------------
/// Given a string containing .ini style configuration of variants, load them
/// into the supported variants for Fairy Stockfish.
///
/// @param config A string containing the ini style variant configuration.
/// Please
///               see
...
///------------------------------------------------------------------------------
bool validateFEN(std::string variantName, std::string fen,
...
///------------------------------------------------------------------------------
std::vector<std::string> to960Uci(std::string variantName,
...
public:
  ...
  Position(std::string _variant, std::string startingFen,
  ...
  ///------------------------------------------------------------------------------
  std::string getSAN(std::string uciMove,
                     Notation notation = Notation::NOTATION_DEFAULT) const;
  ...
  getSANMoves(std::vector<std::string> uciMoves,
              Notation ourNotation = Notation::NOTATION_DEFAULT) const;
  ...
  /// @param showPromoted Whether the fen includes promoted pieces (I'm guessing
  /// here)
  /// @param countStarted No clue. (probably has to do with games that end by
  /// certain
  ///                     types of counts, like repetitions?)
  ///
  /// @return the FEN of the new position
  ///------------------------------------------------------------------------------
  std::string getFEN(bool sFen = false, bool showPromoted = false,
  ...
  ///------------------------------------------------------------------------------
  /// Gets result from a given FEN, considering variant end, checkmate and
  /// stalemate Note that this function will assert there are no legal moves. So
  /// only call it when legalMoves are zero
  ///
  ...
  ///
  /// @return Returns a boolean indicating if the game is an immediate end as
  /// well
  ...
  ///
  /// @return Returns two booleans, one for each player which indicate if that
  /// player
  ...
  ///------------------------------------------------------------------------------
  /// Tests whether there has been at least one repetition of positions since
